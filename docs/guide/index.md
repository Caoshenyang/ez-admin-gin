---
title: 快速启动
description: "五步在本地跑起 EZ Admin Gin，并快速理解它当前已经收敛到什么样的企业级后台底座主线。"
---

# 快速启动

EZ Admin Gin 已经从“功能型后台模板”走到企业级通用后台管理系统底座主线。登录、权限、菜单、配置、日志、文件上传、组织体系、数据权限、前端管理台和部署复用都已经形成主线，当前重点不再是继续改章节边界，而是沿既有骨架继续扩可复用模块，并把交付验证入口收成稳定基线。
...
如果需要改端口或数据库密码，直接编辑该文件。生产环境建议通过环境变量覆盖，参考 [第 9 章：部署、升级与复用](/tutorial/chapter-9/)。
:::
