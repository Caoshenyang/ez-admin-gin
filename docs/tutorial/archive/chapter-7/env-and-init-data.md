---
title: 环境变量与初始化数据
description: "部署环境变量完整参考、配置覆盖机制，以及启动时如何通过迁移自动初始化角色、权限和菜单。"
search: false
---
...
::: warning 历史页说明
这页保留的是旧章节位置下的部署环境变量说明，用来兼容早期引用。

当前教程主线里的 canonical 位置已经切到 [第 9 章：环境变量与初始化数据](../chapter-9/env-and-init-data)。
:::

这一页是部署时的参考手册。部署实操在[部署验证与复用说明](./deployment-and-reuse)中完成，这一页回答三个问题：环境变量有哪些、哪些必须手动改、第一次启动后系统会自动准备好什么。

::: tip 🎯 本页定位
不需要从头读到尾。部署时遇到"这个变量是干什么的"或"迁移到底做了什么"的时候，回来查这一页。
:::
...
服务第一次启动时，会通过 golang-migrate 自动执行 `server/migrations/postgres/`（或 `mysql/`）目录下的 SQL 迁移文件，完成数据库建表和种子数据写入。
...
1. **golang-migrate 版本追踪**：通过 `schema_migrations` 表记录当前迁移版本，已执行的迁移不会重复运行。
2. **种子数据幂等 INSERT**：所有种子数据的 INSERT 语句都使用 `ON CONFLICT DO NOTHING`（PostgreSQL）或 `INSERT IGNORE`（MySQL），即使迁移中途失败后重试，也不会因唯一键冲突而报错。

此外，迁移代码内置了 dirty state 自动修复：如果上次迁移中途崩溃导致 `schema_migrations.dirty = true`，程序会自动解锁并重试，无需手动修复数据库。
:::
...
回到部署实操：[部署验证与复用说明](./deployment-and-reuse)。
