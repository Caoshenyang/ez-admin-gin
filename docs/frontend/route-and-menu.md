...
侧边栏菜单启用手风琴模式，一次只展开一个目录。底部附近的目录展开时，`AppSidebar` 还会在渲染完成后自动把新增子菜单滚入可视区域，避免用户误以为点击没有生效。

如果当前页面属于某个子菜单，父级目录会保留明确的蓝色高亮底色，展开和折叠状态下都能识别当前所在模块。

## 顶部搜索

`AppHeader` 的搜索框从同一份授权菜单树派生索引，支持搜索菜单目录、页面和按钮功能：

- 目录项会跳转到该目录下的第一个可访问页面。
- 页面项会直接跳转对应路由。
- 功能项会跳转到它所属页面，具体按钮仍由页面内权限控制。

搜索支持输入过滤、上下键选择、回车跳转和 ESC 收起。搜索结果只来自当前用户已授权菜单和内置页面，不额外请求后端接口。
