...
1. **创建迁移文件**：在 `migrations/{pgsql,mysql}/` 目录下创建编号命名的 SQL 文件
...
- 根据数据库驱动（`pgsql` 或 `mysql`）加载对应子目录的迁移文件
...
func main() {
	...
	// 执行迁移
	dsn, err := database.DSN(cfg.Database)
	if err != nil {
		log.Fatal("get database DSN", zap.Error(err))
	}

	if err := migrate.Run(cfg.Database.Driver, dsn, migrationsFS, log); err != nil {
