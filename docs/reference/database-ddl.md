...
COMMENT ON COLUMN sys_role.id IS '角色记录主键';
...
```sql [MySQL]
CREATE TABLE `sys_role` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '角色记录主键',
...
COMMENT ON COLUMN sys_user_role.id IS '关系记录主键';
...
```sql [MySQL]
CREATE TABLE `sys_user_role` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '关系记录主键',
