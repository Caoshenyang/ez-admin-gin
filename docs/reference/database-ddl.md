...
<a id="sys-login-log"></a>

## `sys_login_log` 登录日志表

`sys_login_log` 保存后台登录成功和失败记录。它是安全审计记录，不做逻辑删除，也不保存明文密码。
...
| --- | --- |
| `id` | 登录日志主键，数据库自增生成 |
| `user_id` | 用户 ID，对应 `sys_user.id`；用户名不存在时为 `0` |
| `username` | 登录用户名 |
...
::: details 为什么登录日志没有 `updated_at` 和 `deleted_at`
登录日志记录的是登录行为事实，正常情况下不编辑、不逻辑删除。后续如果数据量变大，可以按时间归档或清理。
:::
...
`sys_notice` 保存系统公告。支持启用/禁用控制和逻辑删除。
