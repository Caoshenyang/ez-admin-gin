...
::: info 当前删除能力怎么落地
“删除后不复用唯一值”和“不给删除接口”不是一回事。

当前底座里，用户、角色、菜单、部门、岗位、配置、字典、公告这些管理实体都可以走逻辑删除；真正的保护措施放在 Service 层，例如：

- 删除前检查有没有用户绑定、子节点、角色数据范围或字典项依赖
- 关键对象禁止删除，例如当前登录用户、`super_admin`
- 删除后继续保留历史记录和唯一值占用，避免审计歧义

操作日志、登录日志这类事实记录则继续默认不做单条删除。
:::
