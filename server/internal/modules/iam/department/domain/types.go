...
type CreateRequest struct {
	...
	LeaderUserID *uint                  `json:"leader_user_id" extensions:"x-nullable"`
	Sort         int                    `json:"sort"`
...
const (
	...
	PermissionDelete       = "system:department:delete"
)
...
func NormalizeDepartmentInput(parentID uint, name string, code string, leaderUserID *uint, sort int, status model.DepartmentStatus, remark string) (uint, string, string, *uint, int, model.DepartmentStatus, string, error) {
	...
	if name == "" {
		return 0, "", "", nil, 0, 0, "", errorsx.BadRequest("部门名称不能为空")
	}
	if len(name) > 64 {
		return 0, "", "", nil, 0, 0, "", errorsx.BadRequest("部门名称不能超过 64 个字符")
	}

	code = strings.TrimSpace(code)
	if code == "" {
		return 0, "", "", nil, 0, 0, "", errorsx.BadRequest("部门编码不能为空")
	}
	if len(code) > 64 {
		return 0, "", "", nil, 0, 0, "", errorsx.BadRequest("部门编码不能超过 64 个字符")
	}

	leaderUserID = normalizeLeaderUserID(leaderUserID)

	if status == 0 {
		status = model.DepartmentStatusEnabled
	}
	if !ValidStatus(status) {
		return 0, "", "", nil, 0, 0, "", errorsx.BadRequest("部门状态不正确")
	}

	remark = strings.TrimSpace(remark)
	if len(remark) > 255 {
		return 0, "", "", nil, 0, 0, "", errorsx.BadRequest("备注不能超过 255 个字符")
	}

	return parentID, name, code, leaderUserID, sort, status, remark, nil
}

func normalizeLeaderUserID(leaderUserID *uint) *uint {
	if leaderUserID == nil || *leaderUserID == 0 {
		return nil
	}
...
func BuildResponse(item model.Department) Response {
	return Response{
		...
		Name:         item.Name,
		Code:         item.Code,
		LeaderUserID: normalizeLeaderUserID(item.LeaderUserID),
		Sort:         item.Sort,
		Status:       item.Status,
		Remark:       item.Remark,
