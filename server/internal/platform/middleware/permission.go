...
func Permission(db *gorm.DB, enforcer *authzPlatform.Enforcer, log *zap.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		roleCodes, err := permissionRoleCodes(c, db)
		if err != nil {
			httpx.Error(c, errorsx.Internal("权限校验失败", err), log)
			c.Abort()
			return
		}

		if len(roleCodes) == 0 {
			httpx.Error(c, errorsx.Forbidden("没有权限访问"), log)
			c.Abort()
			return
		}
