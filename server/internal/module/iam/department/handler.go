...
func (h *Handler) List(c *gin.Context) {
	...
	if err != nil {
		writeError(c, err, "查询部门列表失败", h.log)
		return
	}
...
func (h *Handler) Update(c *gin.Context) {
	...
	departmentID, ok := departmentIDParam(c, h.log)
	if !ok {
		return
	}
...
func (h *Handler) UpdateStatus(c *gin.Context) {
	...
	if err := h.service.UpdateStatus(actor, departmentID, req.Status); err != nil {
		writeError(c, err, "更新部门状态失败", h.log)
		return
	}

	response.Success(c, gin.H{"id": departmentID, "status": req.Status})
}
...
func departmentIDParam(c *gin.Context, log *zap.Logger) (uint, bool) {
	rawID := c.Param("id")
	id, err := strconv.ParseUint(rawID, 10, 64)
	if err != nil || id == 0 {
		response.Error(c, apperror.BadRequest("部门 ID 不正确"), log)
		return 0, false
	}
	return uint(id), true
}
