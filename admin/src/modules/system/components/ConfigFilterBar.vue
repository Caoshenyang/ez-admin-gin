...
<template>
  <EzSearchPanel>
    <NInput v-model:value="query.keyword" clearable placeholder="键 / 名称" class="ez-search-field ez-search-field--primary" @keyup.enter="$emit('search')" />
    <NInput v-model:value="query.group_code" clearable placeholder="分组" class="ez-search-field ez-search-field--sm" @keyup.enter="$emit('search')" />
