...
<template>
  <NLayoutHeader bordered class="flex h-16 items-center justify-between bg-[var(--ez-header-bg)] px-5">
    <div class="flex min-w-0 items-center gap-3">
      ...
      <p class="truncate text-[13px] text-[var(--ez-text-secondary)]">{{ breadcrumbText }}</p>
    </div>
    ...
    <div class="flex items-center gap-1.5">
      <NInput
        ...
        clearable
        size="small"
        class="w-[360px] max-[1100px]:hidden"
      >
        <template #prefix>
      ...
      <NDropdown trigger="click" :options="dropdownOptions" @select="(key) => emit('userAction', key)">
        <NButton
          quaternary
          class="gap-1.5 rounded-[var(--ez-radius-control)] border border-[var(--ez-border)] px-3.5 py-2 text-[var(--ez-text-main)] transition-colors hover:border-[var(--ez-primary)] hover:bg-[var(--ez-page-bg)]"
        >
